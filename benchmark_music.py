...
from benchmark import music_2d_refined
from utils_physics import generate_covariance_matrix
import time

def benchmark_music(snr_list, L_snapshots, num_samples, grid_size='standard', output_dir='results'):
    """
    MUSIC评测
    ...
    for snr in snr_list:
        ...
        for _ in tqdm(range(num_samples), desc=f"SNR={snr}dB"):
            ...
            # 运行MUSIC
            t0 = time.time()
            r_est, theta_est = music_2d_refined(R_complex, r_grid, theta_grid, refine=True)
    ...
    output_path = os.path.join(output_dir, f'music_L{L_snapshots}_{grid_size}.json')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='MUSIC独立评测')
