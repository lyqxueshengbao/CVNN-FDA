...
def run_benchmark(L_snapshots=None, num_samples=500, fast_mode=False, music_continuous=False):
    ...
    for snr in snr_list:
        ...
        for sample_idx in tqdm(range(num_samples), desc=f"SNR={snr}", leave=False):
            r_true = np.random.uniform(0, cfg.r_max)
            theta_true = np.random.uniform(cfg.theta_min, cfg.theta_max)
            ...
            errors["CVNN"]["theta"].append((th_est_cvnn - theta_true)**2)
            ...
            r_est_rcnn = pred[0]*cfg.r_max
            th_est_rcnn = pred[1]*(cfg.theta_max-cfg.theta_min)+cfg.theta_min
            errors["Real-CNN"]["r"].append((r_est_rcnn - r_true)**2)
            errors["Real-CNN"]["theta"].append((th_est_rcnn - theta_true)**2)
        ...
        # 输出详细结果表格
        print(f"\n📊 SNR={snr}dB 各算法性能统计:")
        print(f"{'Method':<12} {'RMSE_r (m)':<12} {'RMSE_θ (°)':<12} {'Time (ms)':<12}")
        print("-" * 50)
        for m in methods:
